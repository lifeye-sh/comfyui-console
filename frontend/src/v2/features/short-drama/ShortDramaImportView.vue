...
import { shortDramaApi, type CreativeJob, type ShortDramaDocument, type ShortDramaProject, type StructuredDramaImport, type StructuredImportType } from '@/api/modules'
...
const structuredImports = ref<StructuredDramaImport[]>([])
const structuredFiles = ref<Partial<Record<StructuredImportType, File>>>({})
const structuredPreviews = ref<Partial<Record<StructuredImportType, Record<string, any>>>>({})
const structuredBusy = ref<StructuredImportType | null>(null)
const structuredSteps: Array<{type:StructuredImportType;title:string;description:string}> = [
  {type:'script',title:'1. 剧本主数据',description:'创建第1集、Scene、Shot、角色和道具'},
  {type:'assets',title:'2. 资产提示词',description:'创建角色造型与场景资产版本，并按组号绑定'},
  {type:'video_prompts',title:'3. 视频提示词',description:'写入Shot时长、时间轴分镜、接续状态和完整提示词'},
]
...
async function load(showError = true) {
  try {
    const [projectData, documentData, jobData, structuredData] = await Promise.all([
      shortDramaApi.project(projectId.value), shortDramaApi.documents(projectId.value), shortDramaApi.jobs({ project_id: projectId.value, page_size: 20 }), shortDramaApi.structuredImports(projectId.value),
    ])
    project.value = projectData
    documents.value = documentData
    jobs.value = jobData.items
    structuredImports.value = structuredData
  } catch (event: any) {
...
function chooseStructured(type:StructuredImportType,event:Event){
  const file=(event.target as HTMLInputElement).files?.[0];error.value=''
  if(!file)return
  if(!file.name.toLowerCase().endsWith('.md')){error.value='请选择Markdown（.md）文件';return}
  structuredFiles.value={...structuredFiles.value,[type]:file};structuredPreviews.value={...structuredPreviews.value,[type]:undefined}
}
async function previewStructured(type:StructuredImportType){
  const file=structuredFiles.value[type];if(!file)return
  structuredBusy.value=type;error.value=''
  try{const result=await shortDramaApi.previewStructuredImport(projectId.value,type,file);structuredPreviews.value={...structuredPreviews.value,[type]:result.parsed_data}}
  catch(event:any){error.value=event.response?.data?.detail||'文件预览失败'}finally{structuredBusy.value=null}
}
async function applyStructured(type:StructuredImportType){
  const file=structuredFiles.value[type];if(!file)return
  structuredBusy.value=type;error.value=''
  try{const batch=structuredImports.value.find(item=>item.data_type==='script')?.batch_key;await shortDramaApi.applyStructuredImport(projectId.value,type,file,batch);structuredFiles.value={...structuredFiles.value,[type]:undefined};structuredPreviews.value={...structuredPreviews.value,[type]:undefined};await load()}
  catch(event:any){error.value=event.response?.data?.detail||'数据文件导入失败'}finally{structuredBusy.value=null}
}
...
<template>
  <div class="v2-page import-page">
    ...
    <GlassPanel title="导入已有制作数据" description="按剧本 → 资产提示词 → 视频提示词的顺序分别导入；每份Markdown原文、哈希和解析结果都会完整保留。">
      <div class="structured-grid">
        <article v-for="step in structuredSteps" :key="step.type" class="structured-card">
          <div class="structured-head"><div><strong>{{ step.title }}</strong><p>{{ step.description }}</p></div><StatusBadge :tone="structuredImports.some(item=>item.data_type===step.type&&item.status==='applied')?'success':'neutral'">{{ structuredImports.some(item=>item.data_type===step.type&&item.status==='applied')?'已导入':'待导入' }}</StatusBadge></div>
          <label class="structured-picker"><input type="file" accept=".md,text/markdown" @change="chooseStructured(step.type,$event)"><span>{{ structuredFiles[step.type]?.name || '选择Markdown文件' }}</span></label>
          <div v-if="structuredPreviews[step.type]" class="preview-counts"><span v-for="(value,key) in structuredPreviews[step.type]?.counts" :key="key">{{ key }}：{{ value }}</span></div>
          <div class="structured-actions"><V2Button :disabled="!structuredFiles[step.type]||structuredBusy===step.type" @click="previewStructured(step.type)">预览</V2Button><V2Button variant="primary" :disabled="!structuredFiles[step.type]||!structuredPreviews[step.type]||structuredBusy===step.type" @click="applyStructured(step.type)">确认导入</V2Button></div>
        </article>
      </div>
    </GlassPanel>
...
.structured-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.structured-card{padding:15px;border:1px solid var(--v2-border);border-radius:14px;background:var(--v2-surface-soft)}.structured-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.structured-head p{min-height:38px;margin:5px 0;color:var(--v2-text-muted);font-size:12px}.structured-picker{display:block;margin:12px 0;padding:12px;border:1px dashed var(--v2-border);border-radius:10px;cursor:pointer}.structured-picker input{position:absolute;width:1px;height:1px;opacity:0}.structured-picker span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.preview-counts{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px}.preview-counts span{padding:3px 7px;color:var(--v2-text-muted);background:rgba(255,255,255,.05);border-radius:6px;font-size:11px}.structured-actions{display:flex;justify-content:flex-end;gap:8px}@media(max-width:900px){.structured-grid{grid-template-columns:1fr}}
</style>
