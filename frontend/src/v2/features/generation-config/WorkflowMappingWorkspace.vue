...
const props = defineProps<{
  ...
  textOutputConfig: { node_id?: string; field?: string }
  editing: boolean
  busy: boolean
...
const emit = defineEmits<{
  ...
  'update:textOutputConfig': [value: { node_id?: string; field?: string }]
    ...
    <Transition name="workspace">
      <div v-if="open" class="workspace-layer v2-theme" role="dialog" aria-modal="true" aria-label="工作流 JSON 导入与参数映射">
        ...
        <main v-else class="mapping-page">
          ...
          <section class="output-section"><V2Field label="输出节点映射（JSON）"><textarea :value="outputMapping" rows="4" @input="emit('update:outputMapping', ($event.target as HTMLTextAreaElement).value)" /></V2Field>
            <div class="text-output-config">
              <div class="text-output-header"><b>文本输出</b><small>翻译/图片反推等返回文字的工作流，选择输出节点和属性名，执行结果会保存为文本素材。</small></div>
              <div class="text-output-row">
                <label><span>输出节点</span>
                  <select :value="textOutputConfig?.node_id || ''" @change="emit('update:textOutputConfig', { ...textOutputConfig, node_id: ($event.target as HTMLSelectElement).value })">
                    <option value="">请选择节点</option>
                    <option v-for="node in nodes" :key="node.value" :value="node.value">{{ node.label }}</option>
                  </select>
                </label>
                <label><span>属性名</span>
                  <select :value="textOutputConfig?.field || ''" @change="emit('update:textOutputConfig', { ...textOutputConfig, field: ($event.target as HTMLSelectElement).value })">
