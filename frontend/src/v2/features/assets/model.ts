export type ResourceItem={id:number;owner_id:number|null;folder_id:number|null;media_type:'image'|'video'|'audio'|'text';direction:string;filename:string;mime:string;size:number;width:number|null;height:number|null;duration:number|null;created_at:string}
...
export function mediaLabel(type:string){return type==='image'?'图片':type==='video'?'视频':type==='audio'?'音频':type==='text'?'文本':'文件'}
