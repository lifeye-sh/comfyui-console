...
export type DramaShot = {
  ...
  purpose:string;visual_description:string;action:string;expression:string;dialogue:string;timeline_storyboard:string;video_prompt:string;negative_prompt:string;continuity:string;character_ids:number[]
...
export type ScriptManifest = {id:number;owner_id:number;project_id:number;episode_id:number;version:number;status:string;source_script_revision:number;mode:'novel'|'storyboard';summary:string;total_duration:number;content:{analysis_context?:Record<string,any>;schema_version?:number;story_summary:string;scenes:Array<Record<string,any>>;characters:Array<Record<string,any>>;locations:Array<Record<string,any>>;props:Array<Record<string,any>>};validation_errors:Array<{severity:'p0'|'p1'|'p2';path:string;message:string;category?:string;blocking_stage?:string|null;actionable?:boolean;action?:string;action_label?:string;action_payload?:Record<string,any>}>;lock_version:number;confirmed_at:string|null;created_at:string;updated_at:string}
...
export type DramaPipeline = {skill:Record<string,any>;project_id:number;episode_id:number;stages:Array<{id:string;name:string;status:string}>;requirements:Array<{stable_key:string;entity_type:string;entity_id:number;name:string;grade:string;required:boolean;ready:boolean;resource_id:number|null;reason:string}>;pending_assets:Array<Record<string,any>>;asset_atlas_locked:boolean;spatial_issues:Array<{type:string;scene_id?:number;shot_id?:number;label:string;message:string}>;spatial_storyboard_locked:boolean;prompt_ready:boolean;quality_issues:Array<{level:string;shot_id?:number;message:string}>;counts:Record<string,number>}
export type StructuredImportType = 'script'|'assets'|'video_prompts'
export type StructuredDramaImport = {id:number;owner_id:number;project_id:number;batch_key:string;data_type:StructuredImportType;filename:string;checksum:string;parsed_data:Record<string,any>;status:string;error:string;created_at:string;updated_at:string}
...
export const shortDramaApi = {
  ...
  previewScriptManifestSplit: (projectId:number,episodeId:number,manifestId:number,body:Record<string,unknown>) => http.post(`/short-drama/projects/${projectId}/episodes/${episodeId}/manifests/${manifestId}/split-preview`,body,{baseURL:'/api/v2'}).then(r=>r.data),
  applyScriptManifestSplit: (projectId:number,episodeId:number,manifestId:number,body:Record<string,unknown>) => http.post(`/short-drama/projects/${projectId}/episodes/${episodeId}/manifests/${manifestId}/split-apply`,body,{baseURL:'/api/v2'}).then(r=>r.data as ScriptManifest),
  ...
  unlockScriptManifest: (projectId:number,episodeId:number,manifestId:number) => http.post(`/short-drama/projects/${projectId}/episodes/${episodeId}/manifests/${manifestId}/unlock`,null,{baseURL:'/api/v2'}).then(r=>r.data as ScriptManifest),
  reviewScriptManifest: (projectId:number,episodeId:number,manifestId:number) => http.post(`/short-drama/projects/${projectId}/episodes/${episodeId}/manifests/${manifestId}/review`,null,{baseURL:'/api/v2'}).then(r=>r.data as ScriptManifest),
  deleteScriptManifest: (projectId:number,episodeId:number,manifestId:number) => http.delete(`/short-drama/projects/${projectId}/episodes/${episodeId}/manifests/${manifestId}`,{baseURL:'/api/v2'}).then(r=>r.data as {deleted:boolean}),
  ...
  dramaPipeline: (projectId:number,episodeId:number) => http.get('/short-drama/projects/'+projectId+'/episodes/'+episodeId+'/pipeline',{baseURL:'/api/v2'}).then(r=>r.data as DramaPipeline),
  confirmAssetAtlas: (projectId:number,episodeId:number) => http.post('/short-drama/projects/'+projectId+'/episodes/'+episodeId+'/asset-atlas/confirm',null,{baseURL:'/api/v2'}).then(r=>r.data as DramaPipeline),
  confirmSpatialStoryboard: (projectId:number,episodeId:number) => http.post('/short-drama/projects/'+projectId+'/episodes/'+episodeId+'/spatial-storyboard/confirm',null,{baseURL:'/api/v2'}).then(r=>r.data as DramaPipeline),
  generateVideoPrompts: (projectId:number,episodeId:number,body:Record<string,unknown>) => http.post('/short-drama/projects/'+projectId+'/episodes/'+episodeId+'/video-prompts/generate',body,{baseURL:'/api/v2'}).then(r=>r.data as CreativeJob),
  copyProjectAsset: (projectId:number,entityType:'character'|'location'|'prop',entityId:number) => http.post(`/short-drama/projects/${projectId}/assets/${entityType}/${entityId}/copy`,null,{baseURL:'/api/v2'}).then(r=>r.data as {entity_type:string;entity_id:number;name:string}),
  ...
  structuredImports: (id:number) => http.get(`/short-drama/projects/${id}/structured-imports`,{baseURL:'/api/v2'}).then(r=>r.data as StructuredDramaImport[]),
  previewStructuredImport: (id:number,dataType:StructuredImportType,file:File) => { const body=new FormData();body.append('data_type',dataType);body.append('file',file);return http.post(`/short-drama/projects/${id}/structured-imports/preview`,body,{baseURL:'/api/v2'}).then(r=>r.data as {data_type:StructuredImportType;filename:string;checksum:string;parsed_data:Record<string,any>}) },
  applyStructuredImport: (id:number,dataType:StructuredImportType,file:File,batchKey?:string) => { const body=new FormData();body.append('data_type',dataType);body.append('file',file);if(batchKey)body.append('batch_key',batchKey);return http.post(`/short-drama/projects/${id}/structured-imports/apply`,body,{baseURL:'/api/v2'}).then(r=>r.data as {item:StructuredDramaImport;applied_counts:Record<string,number>;idempotent:boolean}) },
