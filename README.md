...
- V2.1 漫剧制作已升级为 `short-drama-director V6.5` 驱动的 Asset-First 六阶段工作台，覆盖立项与剧本、数字资产包、空间与分镜、视频制作、质检与交付。
...
当前改造基线（2026-09-08）：SQLite 可从空库迁移到 `0035_structured_drama_imports`；导演工作台、结构化导入、资产复制和首尾帧提示词已有专项回归测试；前端 `vue-tsc --noEmit` 与 Vite 生产构建通过。
...
- `short-drama-director V6.5` 六阶段漫剧生产壳、技能指令入口和 P0/P1/P2 质量门禁。
- 三文档结构化导入，完整保存剧本、资产提示词和视频提示词原文并建立 Scene/Shot/资产关联。
- 数字资产包支持角色、场景、道具新增、原子复制、素材版本、角色三视图、Gemini Image 与 ComfyUI 生图。
- 导演工作台支持首帧、尾帧、关键帧、上下文参考图、工作流参数迁移和 ComfyUI 视频候选。

### 漫剧制作更新（2026-09-08）

- 生产流程重构：项目菜单和页面统一为 P0～P5 Asset-First 流程，由 `short-drama-director V6.5` 的 `/写剧本`、`/台词诊断`、`/语速自检`、`/拆资产`、`/做分镜`、`/生成视频提示词` 和 `/审查` 驱动。
- 导入闭环：支持剧本、资产参考图提示词、视频提示词三个 Markdown 文件分阶段导入，保留全文并关联 Scene、Shot 和资产实体。
- 数字资产包：角色、场景、道具可新增和原子复制；支持提示词版本、图片生成任务卡片、角色三视图及素材库选图。
- 导演工作台：关键帧与视频制作合并，支持首帧、尾帧、关键帧和上下文参考，Gemini/ComfyUI 生图及 ComfyUI 视频生成。
- 提示词一致性：工作流切换保留提示词，勾选素材才参与投喂，首尾帧按 `<Picture X>` 写入时间轴分镜。
- SQLite 迁移：新增时间戳默认值、Take scope 和结构化导入迁移，支持从空数据库升级。
...
漫剧模块以 `short-drama-director V6.5 Multi-Agent` 为核心规则源。生产顺序固定为 P0 立项锁定 → P1 剧本门控 → P2 数字资产包 → P3 空间调度与分镜 → P4 视频制作 → P5 质检交付。资产图册是分镜和视频制作的事实来源。

### 入口与完整操作顺序

1. 在 V2 左侧进入“漫剧制作”，创建项目并设置画幅、集数、单集目标时长和视觉风格。
2. 在“立项与剧本”导入小说或剧本文档，执行 `/写剧本`、`/台词诊断` 和 `/语速自检`，确认单集剧本。
3. 生成拍摄清单，检查每个 Shot 的对白、动作、时长、内部时间轴分镜、角色、场景和道具绑定。一个 Shot 是一个视频生成组，可以在 `timeline_storyboard` 中包含多个内部镜头。
4. 在“数字资产包与资产图册”确认角色、场景和道具。可以新增、复制、编辑提示词、生成图片、从素材库选择图片，并为角色维护服装变体和一张三视图资产图。
5. 资产锁定后进入“空间与分镜”，维护空间走位、CAM1～CAM4、动作轴线和镜头连续性。
6. 在“视频制作”选择镜头上下文、首帧、尾帧、关键帧及其他参考素材，选择 ComfyUI 视频工作流，生成或编辑视频提示词后创建视频候选。
7. 在“质检与交付”执行 P0/P1/P2 审查，处理结构、来源、语速、一致性和生成结果问题。

### 核心指令映射

| 技能指令 | 页面操作 | 主要结果 |
| --- | --- | --- |
| `/写剧本` | 立项与剧本 | 五阶门控剧本、场次和对白 |
| `/台词诊断` | 立项与剧本的台词诊断 | 七维诊断及可复核改写建议 |
| `/语速自检` | 立项与剧本的语速检查 | 字数、时长、语速档位和风险 |
| `/拆资产` | 数字资产包与资产图册 | 角色、场景、道具及资产提示词 |
| `/做分镜` | 拍摄清单、空间与分镜 | 文戏微表情流或武戏动力链 PREVIS |
| `/生成视频提示词` | 视频制作 | 模型适配的视频提示词与接续状态 |
| `/审查` | 质检与交付 | P0/P1/P2 三级门禁结果 |

### 三文档结构化导入

“结构化导入”支持按顺序导入同一批次的三个 Markdown 文件，并完整保存原文、解析结果、校验状态和批次关联：

1. **剧本文档**：建立分集、Scene、Shot、对白和时间轴分镜。
2. **资产参考图提示词**：按稳定名称关联角色、场景、道具和资产提示词。
3. **视频提示词**：按组号关联现有 Shot，写入完整提示词、时间轴分镜和接续状态。

导入文件不会互相覆盖原文。后导入的数据通过批次键、稳定名称、场号和组号关联前一步实体。结构或来源完整性错误必须修复后才能确认。

### 数字资产包

- 角色、场景和道具都支持新增与复制。复制会保留业务描述和最新提示词，但不会复制已采用图片、参考图、来源引用和素材版本。
- 角色外观使用合并后的完整描述，不要求分别维护五官、服装等大量空字段；场景也以完整场景描述为主。
- Gemini Image 和 ComfyUI 都可生成角色、场景、道具及关键帧图片。ComfyUI 会按生图类型加载工作流和对应参数表单。
- 图片生成任务以可折叠卡片网格显示；任务完成后会建立素材版本并自动采用，三视图任务只加入角色参考素材。

### 视频制作与提示词

- 镜头图片支持 Gemini Image 和 ComfyUI；视频只通过 ComfyUI 视频工作流生成。
- 更换工作流时，当前提示词会迁移到新工作流的提示词参数，即使两个工作流使用不同参数名。
- 只有勾选的角色、场景、道具、首尾帧和素材库参考图会加入视频输入，并严格按照统一勾选顺序编号为 `<Picture X>`。
- H3 六段外层结构继续保留：`subject_definitions`、`summary`、`retention_analysis`、`detailed_description`、`overall_soundscape`、`non_diegetic_music`。
- `detailed_description` 只包含 `【时间轴分镜】` 和 `【接续状态】`。勾选首帧时，镜头 1 会加入 `画面开始于首帧[场景简述]<Picture X>，`；勾选尾帧时，最后一个内部镜头会加入 `画面结束于尾帧[场景简述]<Picture X>，`。
- 默认负面约束包括禁止肢体穿模、画面文字、改变服装和改变发型颜色，并保存在外层 `Negative`。

### 数据库升级

升级已有部署前先备份 `backend/data`，再执行：
...
- `0033_phase1_timestamp_defaults`：补齐第一阶段 ORM 插入所需的服务器时间默认值。
- `0034_director_take_scopes`：按首帧、尾帧、关键帧和视频范围维护独立候选。
- `0035_structured_drama_imports`：增加三文档分阶段导入记录和关联数据。

禁止只替换代码而跳过迁移，否则会出现 `no such column` 或 `no such table`。
