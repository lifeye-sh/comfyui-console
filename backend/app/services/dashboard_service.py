...
def get_summary(db: Session) -> dict:
    ...
    today_tasks = db.query(Task).filter(Task.created_at >= today).all()
    trend_tasks = db.query(Task).filter(Task.created_at >= trend_start).all()
    task_status_counts = Counter(task.status for task in today_tasks)
    active_task_count = db.query(Task).filter(Task.status.in_(ACTIVE_TASK_STATUSES)).count()
    ...
    recent_tasks = db.query(Task).order_by(Task.id.desc()).limit(8).all()
